...
**Arc 2 — upstream to MetaMask (= S4). ← START HERE next session.** Zero cost, no funds. Now backed by a real **MAINNET** tx (Exa `0xfb19efdb…00eec8`) **plus** the Sepolia S2 receipt — strong, reproducible evidence.
1. **`MetaMask/agent-skills` PR**: a docs/skill workflow "mm as an x402 payer" — the `createMmX402Account` adapter pattern (`sign-typed-data --wait` bridge), the fail-closed `PaymentGuard`, and two reproducible proofs: the **mainnet** Exa run (tx [`0xfb19efdb…00eec8`](https://basescan.org/tx/0xfb19efdb5979a67ad55f824a76176809ce321d632e256f163f4951fc1b00eec8), real USDC vs an independent seller) + the Sepolia leg (tx [`0x382492be…44ee9`](https://sepolia.basescan.org/tx/0x382492be5b9ca4dc4438402747601e16d5f16cbeaafa905ed91bf563fac44ee9)).
2. **Feature request: native `mm x402` command** — cite the shim as the working spec.
3. Include the field findings upstream cares about: default `x402.org/facilitator` is **dead** (`facilitator.x402.rs` is the live v2 one); the **v1-wire `maxAmountRequired` vs v2 `amount`** normalization trap (signs `value: undefined` if missed); **accepts-selection** (a seller's 402 may advertise many rails — Exa lists Base+Solana, QuickNode ~22 — so filter to your target network+asset, never blind-take `accepts[0]`); seller discovery via the CDP **Bazaar** (`GET https://api.cdp.coinbase.com/platform/v2/x402/discovery/resources`, public/no-auth); and that `mm sign-typed-data --wait` (TEE server-wallet, beast mode) is the whole adapter — reference impl = `scripts/mm-x402-account.ts` + `scripts/x402-exa-leg.ts`.
