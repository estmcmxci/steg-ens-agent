...
**Step 6 — Prove the TEE server-wallet signs EIP-712 **headlessly** — the decisive test (agent).** Make-or-break, not a smoke test: does the TEE server-wallet return a usable typed-data signature **without an MFA prompt**? (This is the S5 feasibility gate — §15.7 #4.) Sign a **pinned inert** EIP-712 doc (§15.7 #10) with `--wait`: `mm wallet sign-typed-data --chain-id 8453 --payload '<inert>' --wait --json`, where `domain.verifyingContract = 0x0000000000000000000000000000000000000001`, nonsense `name`, `primaryType: 'StegSmokeTest'` (one string field) — plus a variant that **omits `EIP712Domain` from `types`** (the scheme does). *Exit:* `--wait` resolves to a `0x…` signature (≥130 hex) with **no MFA prompt** (confirms `beast` trading-mode + headless typed-data signing on 2.0.0). **If it demands MFA / can't resolve headlessly:** the headless booking is blocked (S1/S5) → set the trading-mode to `beast`, or scope S1 to an interactive (human-MFA) path (Step 9).
...
| 4 | 🟠 high | **Headless TEE signing must resolve without MFA (the real S5 gate).** Re-scoped (2026-06-30): the payer is the **TEE server-wallet**, not BYOK — so the gate and the payment now sign with the **same TEE EOA** and the old "signer-identity split / BYOK-broken-on-2.0.0 / raw `MM_MNEMONIC`" concern **dissolves**. The live risk is that server-wallet signing is **async** (`pollingId`/`--wait`) and, in `guard` trading-mode, a policy check can demand **MFA** a headless agent can't satisfy. | The adapter's `signTypedData` runs `mm wallet sign-typed-data … --wait` and blocks on the TEE result. **Step 6 is the decisive test** that typed-data signing resolves **headlessly** (`beast` mode, no MFA); if it can't, the headless booking (S1/S5) is blocked → switch to `beast` or scope S1 to an interactive-MFA path. Still assert the active wallet's reverse-ENS `== agent.steg.eth` before settling. |
...
**Build status (2026-06-30).** Step 0 deps installed (pinned) + typecheck green (#15). `scripts/mm-x402-account.ts` + unit tests landed (13/13) → clears **#6, #14, #16** and the **signer-side of #5**. *Still to build before Phase 0:* the **capture-only OAuth/MCP client** (#1 `autoPayment:false`, #12 callback hardening), the **`onPaymentRequested` fail-closed hook** (rest of #5), the **gate/identity wiring + idempotency** (#4/#11/#13), and the **§15.2 session precondition** (#3). Then Phase 0 = zero-spend 402 capture (needs the Travala email + one human OAuth consent).
