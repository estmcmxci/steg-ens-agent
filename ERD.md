...
> **Updated 2026-07-04. Branch `feat/x402-mm-payer`.** The thesis is fully proven, end-to-end and **live**: **Arc 1** (mainnet payer, Exa) + **Arc 3** (steg integration — brain pays x402 sellers behind a per-payment, operator-revocable ENS gate; **the user drove a real payment hands-on through steg-ens.vercel.app**) are **DONE**. Only **Arc 2 (upstream to MetaMask)** remains: post the drafted maintainer follow-up on #23, then the mcp-mode PR (see ⏭ NEXT). No hard unknowns open. All work committed on `feat/x402-mm-payer` through `f743caf`; ERD/memory are the only doc updates pending commit.
...
**⏭ NEXT — Arc 3 (S5 steg) COMPLETE + LIVE-PROVEN through the UI (2026-07-03). The remaining work is all Arc 2 (upstream).** Arc 1 payer-proven (Exa) + Arc 3 done end-to-end (capstone `0x7056c04f…`; on-chain grant on carlos `0x8061e0…`; brain gates via the capability; LIVE revoke→refuse→restore `0x9a73c5…`→REVOKED→`0xca9e9d…`→allowed; §15.7 #2 closed; and the **user drove a real payment hands-on via steg-ens.vercel.app** — item 4). **NEXT ACTIONS (Arc 2, in order):**
> 1. **Post the maintainer follow-up on [#23](https://github.com/MetaMask/agent-skills/issues/23)** — DRAFT READY (≤280, written 2026-07-03), post as an issue comment:
>    `@AyushBherwani1998 friendly nudge on this — I've got a working mm-signed x402 payer settling on Base mainnet, and I'd be glad to PR the MCP transport into `x402_pay.py` (1 of 3 v2 transports; #16 did HTTP). cc @basgys, extends your work. Open to it?`
>    (Ayush first-line = de-facto maintainer; cc @basgys = authored #16; do NOT first-line-tag @chaitanyapotti. estmcmxci = author_association NONE.)
> 2. **If greenlit → open the mcp-mode PR** (see Arc 2 block below for scope). Reference signer impl = `scripts/mm-x402-account.ts` + `scripts/x402-exa-leg.ts` (+ now `scripts/x402-brain-pay.ts`, `scripts/x402-gate.ts`).
> 3. Also drafted this session (optional, unposted): a **~542-char program update** on how the agent runs an x402 payment (mm `sign-typed-data` on the TEE wallet → EIP-3009 → `PAYMENT-SIGNATURE` → facilitator settle; ENS gate per-payment + operator-revocable). Ask the user where it posts.
