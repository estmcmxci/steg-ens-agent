...
| | |
|---|---|
| **Status** | Draft v3 — **implementation underway.** Phase 0 + Phase 1 build DONE; **§9 Sepolia EXECUTE GREEN (S2, 2026-07-02)**; **MAINNET EXECUTE GREEN (2026-07-03)** — the mm TEE payer settled real USDC on Base mainnet vs an independent seller (**Exa**, tx `0xfb19efdb…00eec8`), gate ✓, exact-amount asserted. Payer = steg's ENS-gated **TEE server-wallet** `agent.steg.eth`. Travala's settlement host went **down** mid-Arc-1 (issue #3) → pivoted the *payer* proof to Exa; **Travala stays the eventual real-product S1** once it recovers. **Arc 2 (S4) PROPOSAL POSTED** → [agent-skills#23](https://github.com/MetaMask/agent-skills/issues/23) (pivoted to the MCP-transport gap; the generic payer was already merged upstream as PR #16). **Arc 3 (S5 steg) STEPS 1+2 DONE 2026-07-03** — brain `x402_pay_{preview,execute}` tools + capstone real settlement via the brain tool (tx `0x7056c04f…082c0`, $0.007 Base USDC, gate ✓) + **deployed to Railway prod** with a gated PREVIEW GREEN from the deployed brain (zero-spend). Remaining, **in this order: Arc 3 Step 3 (`x402.payment` capability)**, then contact #23's maintainers (**@AyushBherwani1998**, cc **@basgys**) + open the mcp-mode PR. **Resume → §0.** |
| **Author** | estmcmxci |
| **Date** | 2026-06-29 |
...
3. **`x402.payment` capability** (← NEXT; LAST of Arc 3 — the honest-claim refinement, biggest change) — a new `actionType` in the verifier (`src/types.ts` + `checkPolicy.ts` + `schema.ts` + `evaluateAction`), worker redeploy, a rework of the gate *probe* (`demo-mm.ts`/`demo-request.ts` to carry the real amount/recipient instead of the fixed `erc20.transfer` placeholder), **and a real on-chain policy record** on `agent.steg.eth`, so the operator can revoke *payment* authority specifically — closes §15.7 #2's honest-claim gap (today the gate is an identity kill-switch, not per-payment authorization).
