...
| | |
|---|---|
| **Status** | Draft v3 — **implementation underway.** Phase 0 + Phase 1 build DONE; **§9 Sepolia EXECUTE GREEN (S2, 2026-07-02)**; **MAINNET EXECUTE GREEN (2026-07-03)** — the mm TEE payer settled real USDC on Base mainnet vs an independent seller (**Exa**, tx `0xfb19efdb…00eec8`), gate ✓, exact-amount asserted. Payer = steg's ENS-gated **TEE server-wallet** `agent.steg.eth`. Travala's settlement host went **down** mid-Arc-1 (issue #3) → pivoted the *payer* proof to Exa; **Travala stays the eventual real-product S1** once it recovers. **Arc 2 (S4) PROPOSAL POSTED** → [agent-skills#23](https://github.com/MetaMask/agent-skills/issues/23) (pivoted to the MCP-transport gap; the generic payer was already merged upstream as PR #16). **Arc 3 (S5 steg) COMPLETE 2026-07-03** — brain `x402_pay_{preview,execute}` tools + capstone real settlement (tx `0x7056c04f…082c0`) + Railway prod deploy + the `x402.payment` verifier capability (27 tests) + **on-chain capability published on carlos.steg.eth** (tx `0x8061e0…a039d`) + **the brain now gates payments via the capability**, proven with a **LIVE revoke→refuse→restore through the deployed brain** (revoke `0x9a73c5…b9c5` → REVOKED → restore `0xca9e9d…09fe2` → allowed). §15.7 #2 CLOSED. Remaining: **contact #23's maintainers (**@AyushBherwani1998**, cc **@basgys**) + open the mcp-mode PR (Arc 2)**. **Resume → §0.** |
| **Author** | estmcmxci |
| **Date** | 2026-06-29 |
...
**⏭ NEXT — Arc 3 (S5 steg) COMPLETE (2026-07-03): brain payer tools + capstone execute (tx `0x7056c04f…082c0`) + Railway prod deploy + `x402.payment` verifier capability + on-chain grant on carlos.steg.eth (tx `0x8061e0…a039d`) + brain gates via the capability + LIVE revoke→refuse→restore through the deployed brain (`0x9a73c5…` → REVOKED → `0xca9e9d…` → allowed). §15.7 #2 closed. NEXT = Arc 2 — contact #23's maintainers (**@AyushBherwani1998** primary, cc **@basgys**) + open the mcp-mode PR.** Arc 1 pivoted + payer-proven; **Arc 2's proposal is already POSTED** (issue [#23](https://github.com/MetaMask/agent-skills/issues/23)). Only **after Arc 3** do we contact the #23 maintainers (**@AyushBherwani1998** primary, cc **@basgys**) and open the mcp-mode PR.
...
4. Then: ⏸ **live demo on steg-ens.vercel.app** (held — the plumbing is now all live behind the gate).
