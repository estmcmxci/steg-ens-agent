...
| | |
|---|---|
| **Status** | Draft v3 — **implementation underway.** Phase 0 + Phase 1 build DONE; **§9 Sepolia EXECUTE GREEN (S2, 2026-07-02)**; **MAINNET EXECUTE GREEN (2026-07-03)** — the mm TEE payer settled real USDC on Base mainnet vs an independent seller (**Exa**, tx `0xfb19efdb…00eec8`), gate ✓, exact-amount asserted. Payer = steg's ENS-gated **TEE server-wallet** `agent.steg.eth`. Travala's settlement host went **down** mid-Arc-1 (issue #3) → pivoted the *payer* proof to Exa; **Travala stays the eventual real-product S1** once it recovers. **Arc 2 (S4) PROPOSAL POSTED** → [agent-skills#23](https://github.com/MetaMask/agent-skills/issues/23) (pivoted to the MCP-transport gap; the generic payer was already merged upstream as PR #16). **Arc 3 (S5 steg) STEPS 1+2 DONE + Step 3a/3b DONE 2026-07-03** — brain `x402_pay_{preview,execute}` tools + capstone real settlement via the brain tool (tx `0x7056c04f…082c0`, $0.007 Base USDC, gate ✓) + **deployed to Railway prod** (gated PREVIEW GREEN from the deployed brain); **plus the `x402.payment` verifier capability built (27 tests, `ef88e09`) + deployed to the `steg-verifier` worker.** Remaining: **Arc 3 Step 3c (on-chain publish, PAUSED — $1.00 cap decided; needs signer decision) → 3d (gate-probe rework + revoke demo)**, then contact #23's maintainers (**@AyushBherwani1998**, cc **@basgys**) + open the mcp-mode PR. **Resume → §0.** |
| **Author** | estmcmxci |
| **Date** | 2026-06-29 |
...
**⏭ NEXT — Arc 3 STEPS 1+2 + 3a/3b DONE (2026-07-03): brain payer tools + capstone execute (tx `0x7056c04f…082c0`) + Railway prod deploy + the `x402.payment` verifier capability (27 tests, `ef88e09`) deployed to `steg-verifier`. Next = Arc 3 Step 3c — the IRREVERSIBLE on-chain publish of `auth.{credential,capability,revocation}[x402-payment]` on agent.steg.eth ($1.00 cap decided; OPEN: who signs) — then 3d (gate-probe rework + revoke→refuse demo). Arc 2's PR + maintainer contact come AFTER Arc 3.** Arc 1 pivoted + payer-proven; **Arc 2's proposal is already POSTED** (issue [#23](https://github.com/MetaMask/agent-skills/issues/23)). Only **after Arc 3** do we contact the #23 maintainers (**@AyushBherwani1998** primary, cc **@basgys**) and open the mcp-mode PR.
...
3. **`x402.payment` capability** (LAST of Arc 3 — the honest-claim refinement, closes §15.7 #2). Phased:
   ...
   - **3c ⏸ PAUSED — the on-chain publish (IRREVERSIBLE, resume here).** Publish `records/agent.steg.eth.x402-payment.json` = `auth.credential[x402-payment]` (signer `0x0943…7EE1`) + `auth.capability[x402-payment]` (`x402.payment`, asset = Base USDC `0x8335…2913`, **maxAmount `1000000` = $1.00** ← DECIDED, allowedPayTo `"*"` or pin-to-Exa TBD, network `eip155:8453`) + `auth.revocation[x402-payment]` `{"revoked":false}` sentinel — via `publish-records.ts` (needs extending to emit an x402.payment policy) → ens-cli multicall calldata → signed by agent.steg.eth's controller. **OPEN DECISION: who signs the publish tx** (operator Ledger / `OPERATOR_HOT_KEY`, or the agent's own `mm` wallet).
   - **3d** — rework the gate probe so `x402_pay_execute` probes the REAL `x402.payment` (asset/payTo/amount under credentialId `x402-payment`) → the gate authorizes the *specific* payment, not the fixed `erc20.transfer` placeholder; brain redeploy; then the money-shot: revoke `auth.revocation[x402-payment]` at ENS → the next payment is refused (payment authority revoked independently of transfers).
