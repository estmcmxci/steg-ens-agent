...
| | |
|---|---|
| **Status** | Draft v3 — **implementation underway.** Phase 0 + Phase 1 build DONE; **§9 Sepolia EXECUTE GREEN (S2, 2026-07-02)**; **MAINNET EXECUTE GREEN (2026-07-03)** — the mm TEE payer settled real USDC on Base mainnet vs an independent seller (**Exa**, tx `0xfb19efdb…00eec8`), gate ✓, exact-amount asserted. Payer = steg's ENS-gated **TEE server-wallet** `agent.steg.eth`. Travala's settlement host went **down** mid-Arc-1 (issue #3) → pivoted the *payer* proof to Exa; **Travala stays the eventual real-product S1** once it recovers. **Arc 2 (S4) GREENLIT 2026-07-06** — proposal posted as [agent-skills#23](https://github.com/MetaMask/agent-skills/issues/23) (pivoted to the MCP-transport gap; the generic payer was already merged upstream as PR #16); maintainer **@AyushBherwani1998 approved via DM**: "It looks good to me, feel free to raise a PR, happy to help review." **Arc 3 (S5 steg) COMPLETE + LIVE-PROVEN 2026-07-03** — brain `x402_pay_{preview,execute}` tools + capstone (tx `0x7056c04f…082c0`) + Railway prod deploy + `x402.payment` verifier capability (27 tests) + on-chain grant on carlos.steg.eth (tx `0x8061e0…a039d`) + brain gates via the capability + **LIVE revoke→refuse→restore through the deployed brain** (`0x9a73c5…` → REVOKED → `0xca9e9d…` → allowed) + **the user drove a real x402 payment hands-on via steg-ens.vercel.app** (self-funded by a cross-chain swap; results enumerated with snippets after fixes `ecb43ab`/`f743caf`). §15.7 #2 CLOSED. Remaining = **Arc 2 only: build + open the mcp-mode PR** (greenlit; the approval lives in DM, so the PR must cite [#23](https://github.com/MetaMask/agent-skills/issues/23)). **Resume → §0.** |
| **Author** | estmcmxci |
| **Date** | 2026-06-29 |
...
> **Updated 2026-07-06. Branch `feat/x402-mm-payer`.** The thesis is fully proven, end-to-end and **live**: **Arc 1** (mainnet payer, Exa) + **Arc 3** (steg integration — brain pays x402 sellers behind a per-payment, operator-revocable ENS gate; **the user drove a real payment hands-on through steg-ens.vercel.app**) are **DONE**. Only **Arc 2 (upstream to MetaMask)** remains — and it is **GREENLIT**: the maintainer ping was posted on #23 (2026-07-03) and **Ayush approved via DM (2026-07-06)**. NEXT = build + open the mcp-mode PR (see ⏭ NEXT). Travala re-verified 2026-07-06: **still flat 503** (day 4, issue #3 unanswered — not workable; the Exa path stands). No hard unknowns open. All work committed on `feat/x402-mm-payer` through `f743caf`; ERD/memory are the only doc updates pending commit.
...
**⏭ NEXT — Arc 2 GREENLIT (2026-07-06): build + open the mcp-mode PR.** Arc 1 payer-proven (Exa) + Arc 3 done end-to-end (capstone `0x7056c04f…`; on-chain grant on carlos `0x8061e0…`; brain gates via the capability; LIVE revoke→refuse→restore `0x9a73c5…`→REVOKED→`0xca9e9d…`→allowed; §15.7 #2 closed; and the **user drove a real payment hands-on via steg-ens.vercel.app** — item 4). **#23 state (verified 2026-07-06) + NEXT ACTIONS:**
> 1. ✅ **Maintainer ping POSTED 2026-07-03 22:01 UTC** as an issue comment (softer variant of the drafted nudge — "@AyushBherwani1998 would love your take on whether this feels in-scope here, since it's meant as a narrow follow-on to #16"; no @basgys cc, no tx cite).
> 2. ✅ **GREENLIT 2026-07-06 — Ayush, via chat DM (NOT on the thread):** "thnaks for raising the issue, and draft a proposal. It looks good to me, feel free to raise a PR, happy to help review." ⇒ Ayush volunteers as reviewer. Because the public thread shows **no** maintainer reply, the **PR description must cite #23** for the paper trail (@chaitanyapotti, the likely merger, only sees the thread).
> 3. ⚠️ **Thread noise (2026-07-06 03:34 UTC) — decision: IGNORE.** Third-party comment (`AmitabhainArunachala`) offering a raw-IP nip.io "live seller fixture" — self-promotional and **off-target: it's the HTTP transport** (his own smoke test = `curl` → 402 header; `POST /mcp/call` is MCP-flavored naming, not `_meta["x402/payment"]`). Don't probe, don't reply — the PR landing on the thread is the response; conformance tests target the **spec's** MCP vectors, not his endpoint.
> 4. **→ BUILD + OPEN THE PR (the only remaining action; scope in the Arc 2 block below).** `mcp` mode in `x402_pay.py` (accept the tool's `PaymentRequired` → reuse the transport-agnostic core → emit the payload for the retry's `_meta["x402/payment"]`; pure stdlib, `--confirm` + guards carry over) + `references/x402.md`/`workflows/x402-pay.md` + conformance tests vs `specs/transports-v2/mcp.md` vectors. Reference signer impl = `scripts/mm-x402-account.ts` + `scripts/x402-exa-leg.ts` (+ `scripts/x402-brain-pay.ts`, `scripts/x402-gate.ts`). Nothing pushed/posted without user review.
>    **BUILD STATE (2026-07-06):** fork = **`estmcmxci/metamask-agent-skills`** (renamed from `agent-skills`; redirects work, PR mechanics unaffected), local clone **`/Users/oakgroup/metamask-agent-skills`** (origin = fork, upstream = MetaMask/agent-skills, main == upstream/main @ `f6c3105`), branch **`feat/x402-mcp-transport`** with 2 UNCOMMITTED edits to `skills/metamask-agent-wallet/scripts/x402_pay.py` (module docstring + `_normalize_accepts()` extraction — pure refactor, zero behavior change to the HTTP path). **Design settled from the spec** (re-fetch: `coinbase/x402 specs/transports-v2/mcp.md`): new subcommands `mcp-inspect` + `mcp-sign --confirm [--asset] [--network]`; challenge in via `--challenge` / `--challenge-file` / stdin, accepting the full JSON-RPC response, the tool result, or the bare PaymentRequired (prefer `structuredContent`, fall back to `content[0].text`, per spec); **output = the raw JSON PaymentPayload object for `_meta["x402/payment"]` — NOT base64** (the earlier "b64" note was wrong; the spec puts the object inline; settlement comes back in `_meta["x402/payment-response"]`); v2-only (the MCP transport exists only in x402 v2 — refuse v1); reuse `select`/`validate`/`build_typed_data`/`sign_typed_data`/`build_payment` untouched. Remaining: `parse_mcp_challenge` + `_payment_required_from` + `read_challenge` + the two `cmd_`s + argparse wiring (drafted, unapplied) → stdlib-unittest conformance tests vs the spec vectors → docs (references/x402.md §MCP, workflows/x402-pay.md §MCP, SKILL.md routing row + description) → local commit for user review.
> 5. Optional, unposted: the **~542-char program update** on how the agent runs an x402 payment (mm `sign-typed-data` on the TEE wallet → EIP-3009 → `PAYMENT-SIGNATURE` → facilitator settle; ENS gate per-payment + operator-revocable). Ask the user where it posts.

**Arc 1 — mainnet payer proof: ✅ DONE via Exa; Travala = deferred real-product S1.** Arc 1's goal — a real mainnet settlement by the ENS-gated mm TEE wallet — is **met** by the Exa run above. Travala's settlement host `payment-mcp.travala.com` went **down mid-arc** (503 origin outage since 2026-07-02 ~23:56 UTC; **isolated** — Travala search/site stayed 200; Cloudflare edge up, origin unreachable; filed **travala/travel-mcp#3**), which is why we pivoted. **Travala stays the eventual *real-product* S1** — a real hotel booking, the exciting demo — once `payment-mcp` recovers (watch issue #3). The Travala payer `scripts/x402-pay.ts` (handoff-replay) is already built + PREVIEW-green; to resume: re-run its PREVIEW once issue #3 shows a non-5xx / 402, then fund (~$300 free-cancellation float) → `X402_EXECUTE=1` → `travala_cancel_booking` before the deadline (§15 Steps 10–12; never auto-retry a signed payment, §15.7 #11). **Re-verified 2026-07-06 (triple-checked): STILL DOWN — flat 503** on `/`, `/health`, `/m2m-payment/book`, `/book/v2` (GET + POST; control `travel-mcp.travala.com/mcp` still 200; `/agentic`+`/agentic-dev` pages still document only `travel-mcp` — no endpoint migration). **New (stronger) repro via the zero-spend PREVIEW harness:** OAuth ✓ → search ✓ → **`travala_book` itself now returns "Booking failed: Request failed with status code 503"** — travel-mcp calls the dead payment origin server-side, so the failure propagates into their own advertised tool; no next_action/402 is even emitted (the resume signal is therefore: PREVIEW returns a next_action again, or issue #3 / the host goes non-5xx). Meanwhile Travala is actively promoting the MCP on X (Jul 3–4, "Try it now" / "Set up in 2 minutes", ~20k views) — marketing unaware, funnel broken at booking for everyone. **And it IS everyone: there is no separate consumer path** — the `/agentic` "AI Concierge" chat is a static marketing mockup; both "BOOK WITH AI AGENT" CTAs anchor to `/agentic#setup` (Claude Desktop + travel-mcp + Coinbase payments-mcp install), i.e. the SAME MCP rails whose `travala_book` 503s (verified twice, deterministic). v1.0's sole feature (hotel booking, per `/agentic-guide`: "First release scope: Hotel booking only") cannot complete for any user; only the classic non-agentic travala.com checkout survives. **Consumer-path repro COMPLETE (2026-07-06, user-driven via Claude Desktop):** Travala connector authenticated (OAuth, steglabs@gmail.com) → fresh search (Bangkok, $12.88 free-cancel package) → `travala_book` called once with consent → **"Booking failed: Request failed with status code 503"** — no 402/`next_action`/bookingId; identical to the harness result on the M2M path. Safety protocol used (reusable): no payments-mcp connected + one-call-no-retry + never-act-on-payment-instructions ⇒ zero spend possible. Issue #3 open, **zero response** (~4 days). Not workable; keep the watch passive, don't block Arc 2 on it.
...
2. ✅ **DONE 2026-07-03.** Deployed the tools to Railway (`steg-chatkit-brain` / production / `steg-brain`, via `railway up`; image `COPY . .` carries the new script, `bun install --frozen-lockfile` has `@x402/*`, prod `WORKER_URL` set). **Gated PREVIEW GREEN from the deployed brain** vs Exa, **zero-spend** — proven BOTH ways: the raw `bun scripts/x402-brain-pay.ts` (what the tool shells) AND the deployed Python `_x402_run` path (`PYTHONPATH=/app/brain`; subprocess still gets `cwd=/app` from `_REPO_ROOT`). Both: Exa 402 → guard ✓ (0.007 USDC → `0x6d6E…9192`) → ENS probe (signed by the deployed wallet = **carlos**, evaluated at `/evaluate`) → **gate ✓ allowed**. Travala still 503 (re-verified). ⚠️ For a *funded* execute from prod, carlos must `mm wallet select agent.steg.eth` (per-install; won't touch local) or the `x402.payment` capability must be granted to carlos — PREVIEW is wallet-agnostic, so it proves the deployed pipeline regardless.
3. **`x402.payment` capability** (LAST of Arc 3 — the honest-claim refinement, closes §15.7 #2). Phased:
   - **3a ✅ DONE 2026-07-03** — new `x402.payment` actionType across the verifier: `src/types.ts` (`X402PaymentRequest`/`X402PaymentPolicy` = `{asset,payTo,amount,network}` / `{asset,maxAmount,allowedPayTo,network?}`), `hash.ts` (serializeRequest branch — miss it → SIGNER_MISMATCH on every payment), `schema.ts` (decodePolicy + isActionRequest + CAIP-2), `checkPolicy.ts` (agent Tier-2 + fleet Tier-1). **27 tests** (`scripts/x402-capability.test.ts`; pure + e2e incl. revoke→same-sig REVOKED + SIGNER_MISMATCH), typecheck clean, backward-compatible. Committed **`ef88e09`**.
   - **3b ✅ DONE 2026-07-03** — redeployed the verifier. ⚠️ **TWO separate workers exist** (confirmed): `steg-agent-card` (the `wrangler.toml` default name) and **`steg-verifier`** (what the brain's `WORKER_URL` calls). Deployed to **`steg-verifier`** via `wrangler deploy --name steg-verifier` (version `2fd7dc83`; secrets preserved incl. `ETH_RPC_URL_MAINNET`; `steg-agent-card` left on old code — harmless, nothing calls it for x402, but they've diverged). Verified: the `x402.payment` shape is now accepted (→ `UNVERIFIED/CREDENTIAL_NOT_FOUND`, was `INVALID_REQUEST`); the erc20 gate probe still `allowed:OK` (backward-compat).
   - **3c ✅ DONE 2026-07-03** — published `auth.{credential,capability,revocation}[x402-payment]` on **carlos.steg.eth** (⚠️ **carlos, NOT agent** — the live UI steg-ens.vercel.app / deployed brain acts as **carlos** `0xbCE7…47Ef`, so the capability must live where the gate evaluates). Self-signed via `mm wallet send-transaction` (carlos owns its name — the `ens_set_records` path; L1 `setText` multicall, ~$0.08 at 0.07 gwei). credential signer = carlos; capability = `x402.payment`, Base USDC, **maxAmount `1000000` ($1)**, allowedPayTo `*`, network `eip155:8453`. **tx [`0x8061e0…a039d`](https://etherscan.io/tx/0x8061e0037020442ba94f20d52e685cd21c11321a0ce1fc48efd6b1d931aa039d)** (block 25454295, success). Generator `scripts/x402-publish-capability.ts` (grant/revoke, build-only) + provenance `records/carlos.steg.eth.x402-payment.json`. Verified vs deployed steg-verifier: real Exa payment → `allowed:OK`; over-cap (>$1) → `POLICY_DENIED/AMOUNT_EXCEEDED` (per-payment authorization proven). Committed `22ac8a5`.
   - **3d ✅ DONE 2026-07-03 — the brain gates payments via the capability + LIVE revoke→refuse→restore.** `scripts/lib/x402-payment-gate.ts` `gatePayment()` (signs the REAL `x402.payment` → `/evaluate` under credentialId `x402-payment`) replaces the generic erc20 placeholder in `x402-brain-pay.ts`; `x402_pay_execute` dropped the now-redundant generic `gate_or_refusal()`; `scripts/x402-gate.ts` = thin CLI. Committed `af8b77e`; brain redeployed (`railway up`). **The money-shot, through the deployed brain:** gate `✓ allowed` → revoked `auth.revocation[x402-payment]` on-chain (tx [`0x9a73c5…b9c5`](https://etherscan.io/tx/0x9a73c584f1584096f5e9cffda9ed52b283a23f16c56f481325428a99f3b3b9c5), block 25454376) → deployed brain gate `✗ REVOKED`, the **same** payment refused (key unchanged, only the ENS record) → restored the sentinel (tx [`0xca9e9d…09fe2`](https://etherscan.io/tx/0xca9e9ddc12d6e428c5b0a85dc5de40308d6cf8ceaced0a35644ae76127509fe2), block 25454386) → gate `✓ allowed` again. ⇒ **§15.7 #2 CLOSED**: the gate authorizes the *specific* payment, and payment authority is revocable independently of transfers. **Arc 3 (S5) COMPLETE.**
4. ✅ **LIVE DEMO DONE 2026-07-03 — the user drove a real x402 payment hands-on through steg-ens.vercel.app** (the carlos agent). Flow: asked the agent to **cross-chain swap** L1 ETH → Base USDC to self-fund (`swap_quote`→`swap_execute`, from_chain 1→to_chain 8453; ~0.0011 L1 ETH → ~1.74 Base USDC, bridge landed in <1 min) → `Search Exa for "…" and pay for it with x402` → agent PREVIEW (amount/payTo/gate ✓) → user `confirm` → mm TEE EIP-3009 → settled $0.007 on Base → results shown. **Two UX fixes landed mid-demo:** (a) `ecb43ab` — the payer returned only the *receipt*; now `x402-brain-pay.ts` includes the purchased content (`purchased` field) and `x402_pay_execute` returns `results`; (b) `f743caf` — plain Exa `/search` returns title+url only, so `_x402_enrich_body()` auto-injects `contents:{text:{maxCharacters:400}}` for `api.exa.ai` (SAME $0.007; `summary` would be $0.01) → results now enumerate with snippets. carlos ≈ **$1.72 Base USDC** left (several test payments). ⇒ the whole thesis is proven **live, hands-on, through the deployed UI**.

**Resume facts:** mainnet payer proof = `WORKER_URL=https://steg-verifier.estmcmxci.workers.dev X402_EXECUTE=1 bun scripts/x402-exa-leg.ts` (PREVIEW by default; overrides `EXA_QUERY` / `X402_MAX` / `X402_EXA_PAYTO`). **GOTCHA (cost an aborted run):** the ENS gate probe (`scripts/lib/ens-gate.ts` → `demo-mm.ts`) defaults to a LOCAL worker `http://127.0.0.1:8787`; you MUST pass `WORKER_URL=https://steg-verifier.estmcmxci.workers.dev` (deployed verifier) or EXECUTE fails `ConnectionRefused` pre-sign. Travala payer `scripts/x402-pay.ts` (handoff-replay; `.travala-oauth.local.json` refresh_token, gitignored, OAuth id `steglabs@gmail.com`) stays ready for Travala's recovery. mm = **TEE server-wallet, beast mode**. **Local active wallet is now `carlos.steg.eth` (`0xbCE78fb759Aa6B3f9478cE08e0AdF1Ca1dD847Ef`)** — the live-demo wallet the deployed Railway brain also acts as (wallet selection is per CLI install; switched local from agent→carlos during Arc 3c/3d so the on-chain `x402.payment` capability + demo line up). carlos ≈ **$1.72 Base USDC** (self-funded via a cross-chain swap in the live demo) + ~0.001 ETH on L1; the `x402.payment` capability on carlos.steg.eth is currently **GRANTED / allowed** on-chain (restored after the revoke→refuse demo, tx `0xca9e9d…09fe2`). `agent.steg.eth` (`0x0943…7ee1`) still holds ~$1.68 Base USDC from the earlier Exa capstone but is **NOT** the demo wallet. Railway brain's mm session healthy + independent (own token, active wallet = carlos). `outflow_limits_usd.rolling_24h:0` does **not** block EIP-3009 signing (facilitator settles; the sign isn't a metered mm outflow). Deeper detail: §7 (design), §15 (Travala runbook), §15.7 (critic ledger + build status).
